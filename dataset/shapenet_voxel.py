...
from .binvox_rw import read_as_3d_array_from_path


SYNSET_DICT_DIR = Path(__file__).resolve().parent


class ShapeNetVoxel(ShapeNetBase):
    """
    ...
    def __init__(
        ...
        if version not in [2]:
            raise ValueError("Version number must be 2.")
        self.model_dir = "models/model_normalized.solid.binvox"
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        voxels_loaded = read_as_3d_array_from_path(model_path, fix_coords=False)
        voxels = torch.tensor(voxels_loaded.data, dtype=torch.float).unsqueeze(0)
        return voxels


class ShapeNetVoxel32(ShapeNetBase):
    """
    This class loads ShapeNetCore from a given directory into a Dataset object.
    ShapeNetCore is a subset of the ShapeNet dataset and can be downloaded from
    https://www.shapenet.org/.
